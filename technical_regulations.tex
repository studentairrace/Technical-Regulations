...
\\ v0.3 - Improved Release version (18.01.24): 
\begin{itemize}
  ...
  \item Changed Rule 11.0.1: Single Causes of Failure are now allowed on the vehicle. But teams are recommended to avoid them.
  \item Changed rule 5.2.1: MAJOR CHANGE: Drones are now allowed to also only use 4 Powerunits/motors.
\end{itemize}
...
\begin{enumerate}
  \item The aircraft must have a minimum of 4 and may not have more than 16 power units. However 6-8 powerunits are recommended.
...
\begin{enumerate}
  \item The design and construction of the Hyperdrone should aim to avoid single points of failure that would result in a crash with a catastrophic outcome. Through design calculations, simulations, and testing, teams are able to demonstrate (in the safety report) that the vehicle can safely operate even in a failure.  \\ \\
  This may include:
    \begin{itemize}
